...
async function verifyWithFlutterwave(reference: string): Promise<{ ok: boolean; amount?: number }> {
  ...
  if (data.status !== 'success' || data.data?.status !== 'successful') return { ok: false }
  return { ok: true, amount: Number(data.data.amount) }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const verification = await verifyWithFlutterwave(reference)
    if (!verification.ok) {
